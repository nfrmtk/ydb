...
В отличие от традиционных вторичных индексов, оптимизирующих поиск по равенству или диапазону, векторные индексы позволяют выполнять [векторный поиск](../query_execution/vector_search.md) на основе функций расстояния или схожести.

## Полнотекстовый индекс

[Полнотекстовый индекс](../../dev/fulltext-indexes.md) является особым видом вторичного индекса.

В отличие от традиционных вторичных индексов, оптимизирующих поиск по равенству или диапазону, полнотекстовые индексы позволяют масштабируемо выполнять текстовый поиск по словам и фразам (а при использовании [N-грамм](https://ru.wikipedia.org/wiki/N-грамм) — и по подстрокам). См. также: [Полнотекстовый поиск](../query_execution/fulltext_search.md).

## Блум-индекс

[Блум-индекс](../../dev/bloom-skip-indexes.md) — частный случай [локального индекса](../glossary.md#local-index).

В отличие от глобальных вторичных и полнотекстовых индексов, он не задаётся через `VIEW` в запросе: `VIEW` в KQP подключает отдельную индексную структуру, а локальный блум так не разворачивается — пропуск фрагментов выполняется при чтении основной таблицы на стороне хранилища. См. также: [локальные индексы](../query_execution/local_indexes.md).

## min_max индекс

[min_max индекс](../../dev/min_max-skip-indexes.md) — еще один частный случай [локального индекса](../glossary.md#local-index).  

min_max индекс оптимизирует поиск значения или поиск по диапазону подобно традиционному вторичному индексу.
